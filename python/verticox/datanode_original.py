...
class DataNode(DataNodeServicer):
    ...
    def __init__(
        ...
        self.sum_Dt = self._compute_sum_Dt(Dt, all_features)
...
def serve_standalone(*, data_path, address, timeout):
    ...
    # Last two columns are assumed to be outcome
    time_column = columns[-1]
    outcome_column = columns[-2]

    event_times = data[time_column].values
    event_happened = data[outcome_column].values

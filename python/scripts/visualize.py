...
fig.add_trace(
    go.Scatter(
        ...
        name="Lz",
        mode="lines+markers",
        marker={"color": lz_loop, "cmin": 0, "cmax": 1},
    ),
    row=1,
    col=1,
)
fig.add_trace(
    {"y": z_diff, "name": "z diff", "mode": "lines+markers", "type": "scatter"},
    row=2,
    col=1,
)

fig.add_trace(
    {"y": sigma_diff, "name": "sigma diff", "mode": "lines+markers", "type": "scatter"},
    row=3,
    col=1,
)

fig.add_trace(
    {"y": [], "name": "Mean Absolute Error", "mode": "lines+markers"}, row=4, col=1
)

fig.add_bar(x=beta_df["name"], y=beta_df["value"], name=BETA_BAR, row=5, col=1)
fig.add_bar(x=beta_df["name"], y=beta_df["value"], name=TARGET_BAR, row=5, col=1)
...
def update_graph_live(n):
    ...
    fig.update_traces(
        {"x": new_df["name"], "y": new_df["value"]}, selector={"name": BETA_BAR}
    )
    fig.update_traces(
        {"x": target_df["name"], "y": target_df["value"]}, selector={"name": TARGET_BAR}
    )
...
@app.callback(
    (Output("converged", "children"), Output("interval-component", "disabled")),
